...
module.exports = () => {
	return gulp.task('html:compile-templates', () => {
		...
		notifier.success(`Emitty changed file -> ${global.emittyChangedFile || 'new build start'}`, {
			notStream: true
		});
		...
		new Promise((resolve, reject) => {
			emitty.scan(global.emittyChangedFile).then(() => {
				...
				.pipe(gulpif(tars.options.watch.isActive, emitty.stream(global.emittyChangedFile || '')))
				.pipe(debug({
					title: 'pug:'
				}))
